      ...
      class PooledNetRequester
        ...
        class << self
          ...
          def calibrate_socket_timeout(conn, deadline)
            timeout = deadline - OpenTransit::Internal::Util.monotonic_secs
          ...
          def build_request(request, &blk)
            ...
            in String
              ...
              req.body_stream = OpenTransit::Internal::Util::ReadIOAdapter.new(body, &blk)
            in StringIO
        ...
        private def with_pool(url, deadline:, &blk)
          origin = OpenTransit::Internal::Util.uri_origin(url)
          timeout = deadline - OpenTransit::Internal::Util.monotonic_secs
          pool =
        ...
        def execute(request)
          ...
          enum = Enumerator.new do |y|
            ...
            raise OpenTransit::Errors::APITimeoutError.new(url: url, request: req)
          rescue StandardError
            raise OpenTransit::Errors::APIConnectionError.new(url: url, request: req)
          end
          # rubocop:enable Metrics/BlockLength

          conn, _, response = enum.next
          body = OpenTransit::Internal::Util.fused_enum(enum, external: true) do
            finished = true
            tap do
              enum.next
