...
module OpenTransit
  ...
  module Converter
    ...
    class << self
      # @private
      #
      # @param spec [Hash{Symbol=>Object}, Proc, OpenTransit::Converter, Class] .
      #
      ...
      def type_info(spec)
        ...
        in Proc
          spec
        in OpenTransit::Converter | Class
          -> { spec }
        in true | false
          -> { OpenTransit::BooleanModel }
      ...
      #
      # @param target [OpenTransit::Converter, Class]
      # @param value [Object]
      #
      # @return [Object]
      #
      ...
      def try_strict_coerce(target, value)
        ...
        in Class
          ...
          in [-> { _1 <= Date || _1 <= Time }, String]
            OpenTransit::Util.suppress(ArgumentError, Date::Error) do
  ...
  class Unknown
    ...
    # @return [Boolean]
    #
    def self.==(other) = other.is_a?(Class) && other <= OpenTransit::Unknown
  ...
  class BooleanModel
    ...
    # @return [Boolean]
    #
    def self.==(other) = other.is_a?(Class) && other <= OpenTransit::BooleanModel
  ...
  class Enum
    ...
    #
    def self.==(other)
      other.is_a?(Class) && other <= OpenTransit::Enum && other.values.to_set == values.to_set
    end
  ...
  class Union
    ...
    # @private
    #
    # @param key [Symbol, Hash{Symbol=>Object}, Proc, OpenTransit::Converter, Class]
    #
    # @param spec [Hash{Symbol=>Object}, Proc, OpenTransit::Converter, Class] .
    #
    #   @option spec [NilClass, TrueClass, FalseClass, Integer, Float, Symbol] :const
    ...
    private_class_method def self.variant(key, spec = nil)
      variant_info =
        case key
        in Symbol
          [key, OpenTransit::Converter.type_info(spec)]
        in Proc | OpenTransit::Converter | Class | Hash
          [nil, OpenTransit::Converter.type_info(key)]
        end
    ...
    # @param value [Object]
    #
    # @return [OpenTransit::Converter, Class, nil]
    #
    private_class_method def self.resolve_variant(value)
      case [@discriminator, value]
      in [_, OpenTransit::BaseModel]
        value.class
      in [Symbol, Hash]
        key =
        ...
        resolved.nil? ? OpenTransit::Unknown : resolved.call
      else
        nil
      end
    end
    ...
    #
    def self.==(other)
      other.is_a?(Class) && other <= OpenTransit::Union && other.variants == variants
    end
    ...
    def self.coerce(value)
      if (variant = resolve_variant(value))
        return OpenTransit::Converter.coerce(variant, value)
      end
      ...
      known_variants.each do |_, variant_fn|
        ...
        case OpenTransit::Converter.try_strict_coerce(variant, value)
        in [true, coerced, _]
          return coerced
        in [false, true, score]
      ...
      variant.nil? ? value : OpenTransit::Converter.coerce(variant, value)
    end
    ...
    def self.dump(value)
      if (variant = resolve_variant(value))
        return OpenTransit::Converter.dump(variant, value)
      end
  ...
  class ArrayOf
    ...
    # @return [Boolean]
    #
    def ==(other) = other.is_a?(OpenTransit::ArrayOf) && other.item_type == item_type
    ...
    def coerce(value)
      ...
      in Enumerable unless value.is_a?(Hash)
        value.map { |item| OpenTransit::Converter.coerce(type, item) }
      else
        value
      end
    end
    ...
    def dump(value)
      ...
      case value
      in Enumerable unless value.is_a?(Hash)
        value.map { |item| OpenTransit::Converter.dump(type, item) }.to_a
      else
        value
      end
    end
    ...
    def try_strict_coerce(value)
      ...
      in Array
        ...
        mapped =
          value.map do |item|
            case OpenTransit::Converter.try_strict_coerce(type, item)
            in [true, coerced, score]
              tally += score
              coerced
            in [false, true, score]
    ...
    # @private
    #
    # @return [OpenTransit::Converter, Class]
    #
    protected def item_type = @item_type_fn.call

    # @private
    #
    # @param type_info [Hash{Symbol=>Object}, Proc, OpenTransit::Converter, Class]
    #
    ...
    def initialize(type_info, spec = {})
      @item_type_fn = OpenTransit::Converter.type_info(type_info || spec)
    end
  end
  ...
  class HashOf
    ...
    # @return [Boolean]
    #
    def ==(other) = other.is_a?(OpenTransit::HashOf) && other.item_type == item_type
    ...
    def coerce(value)
      ...
      in Hash
        value.to_h do |key, val|
          coerced = OpenTransit::Converter.coerce(type, val)
    ...
    def dump(value)
      ...
      in Hash
        value.transform_values do |val|
          OpenTransit::Converter.dump(type, val)
        end
      else
        value
      end
    ...
    def try_strict_coerce(value)
      ...
      in Hash
        ...
        mapped =
          value.transform_values do |val|
            case OpenTransit::Converter.try_strict_coerce(type, val)
            in [true, coerced, score]
              tally += score
              coerced
            in [false, true, score]
  ...
  class BaseModel
    ...
    def self.known_fields
      @known_fields ||= (self < OpenTransit::BaseModel ? superclass.known_fields.dup : {})
    end
    ...
    private_class_method def self.add_field(name_sym, required:, type_info:, spec:)
      type_fn, info =
        case type_info
        in Proc | Class | OpenTransit::Converter
          [OpenTransit::Converter.type_info({**spec, union: type_info}), spec]
        in Hash
          [OpenTransit::Converter.type_info(type_info), type_info]
        end
      ...
      define_method(name_sym) do
        ...
        OpenTransit::Converter.coerce(field_type, value)
      rescue StandardError
        name = self.class.name.split("::").last
        raise OpenTransit::ConversionError.new(
    ...
    # @private
    #
    # @param value [OpenTransit::BaseModel, Hash{Object=>Object}, Object]
    #
    # @return [OpenTransit::BaseModel, Object]
    #
    def self.coerce(value)
      case (coerced = OpenTransit::Util.coerce_hash(value))
      in Hash
        new(coerced)
      else
        value
    ...
    # @private
    #
    # @param value [OpenTransit::BaseModel, Object]
    #
    # @return [Hash{Object=>Object}, Object]
    #
    def self.dump(value)
      unless (coerced = OpenTransit::Util.coerce_hash(value)).is_a?(Hash)
        return value
      end
      ...
      values = coerced.filter_map do |key, val|
        ...
        in nil
          ...
          in :coerce
            ...
            target = type_fn.call
            [api_name, OpenTransit::Converter.dump(target, val)]
          end
        end
      end.to_h
    ...
    def self.try_strict_coerce(value)
      ...
      known_fields.each_value do |field|
        ...
        in [_, true]
          ...
          case OpenTransit::Converter.try_strict_coerce(target, item)
          in [true, coerced, score]
            tally += score
    ...
    #
    # @param data [Hash{Symbol=>Object}, OpenTransit::BaseModel]
    #
    def initialize(data = {})
      case (coerced = OpenTransit::Util.coerce_hash(data))
      in Hash
        @data = coerced.transform_keys(&:to_sym)
      else
        raise ArgumentError.new("Expected a #{Hash} or #{OpenTransit::BaseModel}, got #{data.inspect}")
      end
    end
