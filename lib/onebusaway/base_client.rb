...
module Onebusaway
  ...
  class BaseClient
    ...
    # @private
    #
    # @return [Onebusaway::PooledNetRequester]
    ...
    def initialize(
      ...
      @requester = Onebusaway::PooledNetRequester.new
      @headers = Onebusaway::Util.normalized_headers(
        {
          "X-Stainless-Lang" => "ruby",
          "X-Stainless-Package-Version" => Onebusaway::VERSION,
      ...
      @base_url = Onebusaway::Util.parse_uri(base_url)
    ...
    #
    #   @option req [Onebusaway::Converter, Class, nil] :model
    #
    ...
    private def build_request(req, opts)
      ...
      path = Onebusaway::Util.interpolate_path(uninterpolated_path)
      ...
      body =
        ...
        in :get | :head | :options | :trace
          ...
          Onebusaway::Util.deep_merge(*[req[:body], opts[:extra_body]].compact)
        end

      url = Onebusaway::Util.join_parsed_uri(@base_url, {**req, path: path})
      headers, encoded = Onebusaway::Util.encode_content(headers, body)
    ...
    private def should_retry?(status, headers:)
      coerced = Onebusaway::Util.coerce_boolean(headers["x-should-retry"])
      case [coerced, status]
      in [true | false, _]
        coerced
    ...
    private def retry_delay(headers, retry_count:)
      ...
      span = retry_header && Onebusaway::Util.suppress(ArgumentError) do
    ...
    private def follow_redirect(request, status:, location_header:)
      ...
      location =
        Onebusaway::Util.suppress(ArgumentError) do
          URI.join(url, location_header)
        end

      unless location
        message = "Server responded with status #{status} but no valid location header."
        raise Onebusaway::APIConnectionError.new(url: url, message: message)
      end
      ...
      # from undici
      if Onebusaway::Util.uri_origin(url) != Onebusaway::Util.uri_origin(location)
    ...
    private def send_request(request, redirect_count:, retry_count:, send_retry_header:)
      ...
      rescue Onebusaway::APIConnectionError => e
        status = e
      end
      ...
      in Onebusaway::APIConnectionError if retry_count >= max_retries
        raise status
      in (400..) if no_retry || (response && !should_retry?(status, headers: response))
        body = Onebusaway::Util.decode_content(response, suppress_error: true)

        raise Onebusaway::APIStatusError.for(
          url: url,
          status: status,
          body: body,
          request: nil,
      ...
      in (400..) | Onebusaway::APIConnectionError
    ...
    #   @option req [Onebusaway::Converter, Class, nil] :model
    #
    #   @option req [Onebusaway::RequestOptions, Hash{Symbol=>Object}, nil] :options
    #
    # @param response [nil]
    #
    # @return [Object]
    ...
    private def parse_response(req, response)
      parsed = Onebusaway::Util.decode_content(response)
      unwrapped = Onebusaway::Util.dig(parsed, req[:unwrap])

      page = req[:page]
      model = req.fetch(:model, Onebusaway::Unknown)
      case [page, model]
      in [Class, Class | Onebusaway::Converter | nil]
        page.new(client: self, req: req, headers: response, unwrapped: unwrapped)
      in [nil, Class | Onebusaway::Converter]
        Onebusaway::Converter.coerce(model, unwrapped)
      in [nil, nil]
        unwrapped
      end
    end
    ...
    def request(req)
      ...
      Onebusaway::RequestOptions.validate!(opts)
    ...
    #
    def inspect
      base_url = Onebusaway::Util.unparse_uri(@base_url)
