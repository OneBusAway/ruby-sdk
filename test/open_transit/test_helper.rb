...
class OpenTransit::Test::SingletonClient < OpenTransit::Client
  include Singleton

  TEST_API_BASE_URL = ENV.fetch("TEST_API_BASE_URL", "http://localhost:4010")

  def initialize
    super(base_url: OpenTransit::Test::SingletonClient::TEST_API_BASE_URL, api_key: "My API Key")
  end
end
...
class OpenTransit::Test::ResourceTest < Minitest::Test
  def async?
  ...
  def before_all
    super
    @onebusaway_sdk = OpenTransit::Test::SingletonClient.instance
  end
