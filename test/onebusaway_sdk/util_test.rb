...
class OnebusawaySDK::Test::UtilDataHandlingTest < Minitest::Test
  def test_left_map
    assert_pattern do
      OnebusawaySDK::Util.deep_merge({a: 1}, nil) => nil
    end
  end

  def test_right_map
    assert_pattern do
      OnebusawaySDK::Util.deep_merge(nil, {a: 1}) => {a: 1}
    end
  end

  def test_disjoint_maps
    assert_pattern do
      OnebusawaySDK::Util.deep_merge({b: 2}, {a: 1}) => {a: 1, b: 2}
    end
  end

  def test_overlapping_maps
    assert_pattern do
      OnebusawaySDK::Util.deep_merge({b: 2, c: 3}, {a: 1, c: 4}) => {a: 1, b: 2, c: 4}
    end
  end

  def test_nested
    assert_pattern do
      OnebusawaySDK::Util.deep_merge({b: {b2: 1}}, {b: {b2: 2}}) => {b: {b2: 2}}
    end
  end

  def test_nested_left_map
    assert_pattern do
      OnebusawaySDK::Util.deep_merge({b: {b2: 1}}, {b: 6}) => {b: 6}
    end
  end

  def test_omission
    merged = OnebusawaySDK::Util.deep_merge(
      {b: {b2: 1, b3: {c: 4, d: 5}}},
      {b: {b2: 1, b3: {c: OnebusawaySDK::Util::OMIT, d: 5}}}
    )
  ...
  def test_dig
    assert_pattern do
      OnebusawaySDK::Util.dig(1, nil) => 1
      OnebusawaySDK::Util.dig({a: 1}, :b) => nil
      OnebusawaySDK::Util.dig({a: 1}, :a) => 1
      OnebusawaySDK::Util.dig({a: {b: 1}}, [:a, :b]) => 1

      OnebusawaySDK::Util.dig([], 1) => nil
      OnebusawaySDK::Util.dig([nil, [nil, 1]], [1, 1]) => 1
      OnebusawaySDK::Util.dig({a: [nil, 1]}, [:a, 1]) => 1
      OnebusawaySDK::Util.dig([], 1.0) => nil

      OnebusawaySDK::Util.dig(Object, 1) => nil
      OnebusawaySDK::Util.dig([], 1.0, 2) => 2
      OnebusawaySDK::Util.dig([], 1.0) { 2 } => 2
    end
  end
end
...
class OnebusawaySDK::Test::UtilUriHandlingTest < Minitest::Test
  def test_parsing
    %w[
      ...
      parsed = OnebusawaySDK::Util.parse_uri(url)
      unparsed = OnebusawaySDK::Util.unparse_uri(parsed).to_s

      assert_equal(url, unparsed)
      assert_equal(parsed, OnebusawaySDK::Util.parse_uri(unparsed))
    end
  end
  ...
  def test_joining
    cases = [
      [
        "h://a.b/c?d=e",
        "h://nope/ignored",
        OnebusawaySDK::Util.parse_uri("h://a.b/c?d=e")
      ],
      [
        "h://a.b/c?d=e",
        "h://nope",
    ...
    cases.each do |expect, lhs, rhs|
      assert_equal(
        URI.parse(expect),
        OnebusawaySDK::Util.join_parsed_uri(
          OnebusawaySDK::Util.parse_uri(lhs),
          rhs
        )
      )
    end
  ...
  def test_joining_queries
    ...
    cases.each do |path, expected|
      assert_equal(
        URI.parse(expected),
        OnebusawaySDK::Util.join_parsed_uri(
          OnebusawaySDK::Util.parse_uri(base_url),
          {path: path}
        )
      )
    end
...
class OnebusawaySDK::Test::UtilFormDataEncodingTest < Minitest::Test
  ...
  def test_file_encode
    ...
    cases.each do |body, val|
      encoded = OnebusawaySDK::Util.encode_content(headers, body)
...
class OnebusawaySDK::Test::UtilIOAdapterTest < Minitest::Test
  ...
  def test_copy_write
    ...
    cases.each do |input, expected|
      enum = OnebusawaySDK::Util.writable_enum do |y|
...
class OnebusawaySDK::Test::UtilFusedEnumTest < Minitest::Test
  def test_closing
    arr = [1, 2, 3]
    once = 0
    fused = OnebusawaySDK::Util.fused_enum(arr.to_enum) do
      once = once.succ
    end
  ...
  def test_rewind_chain
    once = 0
    fused = OnebusawaySDK::Util.fused_enum([1, 2, 3].to_enum) do
      once = once.succ
    end
      .lazy
      .map(&:succ)
  ...
  def test_close_fused
    ...
    OnebusawaySDK::Util.close_fused!(fused)
  ...
  def test_closed_fused_extern_iteration
    ...
    fused = OnebusawaySDK::Util.fused_enum(enum)
    first = fused.next
